...
The schme operates From 3 to 31 August.  Customers get a 50% discount, up to a maximum of £10 per person, when you eat in at restaurants that are registered with the Eat Out to Help Out Scheme.
...
The HMRC data only provides the address information.  To get the coordinates need for the map I use the National Statistics Postcode Lookup (NSPL) provide free by the Office of National Statistics (ONS).  The NSPL allows you to identify a lattitude and Longitude for the postcodes in the EA2HO data.
...
The NSPL is a great tool for UK statistical geographies.  It can be downloaded freely from the [ONS Open Geography Portal](https://geoportal.statistics.gov.uk/datasets/national-statistics-postcode-lookup-may-2020).  I used the may 2020 version which was the latest available.  I downloaded it to the data folder.  The file is too big to upload to Github so you will need to download it yourself.
...
In this section I format the postcodes in the restaurant data to ensure they join with the NSPL postcodes.  I then combine the address columns into a single column which can be used in a popup on the map.  
